...
Math.Tan(radians)       // 正切（= Sin / Cos，Cos 为零时抛异常）
...
Math.Atan(value)        // 反正切
Math.Atan2(fy, fx)      // 四象限反正切，返回 (-π, π]
```
...
FixedPoint rad2 = Math.DegreesToRadians(degrees);   // degrees * π / 180
FixedPoint deg2 = Math.RadiansToDegrees(radians);   // radians * 180 / π
```
