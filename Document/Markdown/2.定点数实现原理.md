...
```csharp
// 被除数左移 10 位（= 乘以 1024），保留更多精度
// 再除以除数的内部值，得到 a/b 的 Q10 表示
...
{
    if (value.FixedValue >= 0) return value;
...
Q10 格式中，内部值的低 10 位存储小数部分，高位存储整数部分。`~FRAC_MASK` 就是 `~0x3FF`（低 10 位全 0、其余位全 1），对内部值做位与运算后，低 10 位被清零，只保留整数部分。

用两个例子展示它在正数和负数上的行为：

```text
Floor(2.7)：
  内部值 = 2765（2.7 × 1024，四舍五入）
  二进制：0000...0000 10 1100 1101 01
                       ↑整数部分    ↑小数部分(低10位)
  & ~0x3FF → 清零低 10 位
  结果：  0000...0000 10 1100 0000 00 = 2048
  2048 / 1024 = 2.0 ✓

Floor(-2.3)：
  内部值 = -2355（-2.3 × 1024，四舍五入）
  补码：  1111...1111 01 1011 0010 01（-2355 的补码表示）
  & ~0x3FF → 清零低 10 位
  结果：  1111...1111 01 1000 0000 00 = -3072
  -3072 / 1024 = -3.0 ✓
```

关键点在于负数。`-2.3` 做 Floor 应该得到 `-3`（向负无穷方向），而不是 `-2`。在补码表示中，清零低位恰好天然地向负无穷方向取整——因为清零小数部分后，补码值变得更小（更负），这正好是 Floor 的语义。不需要任何条件分支，一次位与操作就完成了。
...
牛顿法的收敛速度高度依赖初始值的质量。如果初始值离真实答案差 1000 倍，需要很多次迭代才能收敛；如果只差 2 倍以内，几次迭代就够了。
...
// 计算 v 的位长度（即 v 需要多少个二进制位来表示）
int bits = 0;
long temp = v;
while (temp > 0) { bits++; temp >>= 1; }

// 初始估值：sqrt(v × MULTIPLE) ≈ 2^((bits + BIT_MOVE_COUNT) / 2)
long estimate = 1L << ((bits + FixedPoint.BIT_MOVE_COUNT) / 2);
```

原理：一个 n 位的二进制数大致等于 2ⁿ（准确地说在 [2ⁿ⁻¹, 2ⁿ) 范围内）。`v` 是 `bits` 位，`v × MULTIPLE` 是 `bits + 10` 位，那么 `√(v × MULTIPLE) ≈ 2^((bits + 10) / 2)`。
...
求 √9.0：
  v = 9216（9.0 × 1024）
  v × MULTIPLE = 9216 × 1024 = 9437184

  v 的位长度：9216 的二进制 = 10 0100 0000 0000，共 14 位
  estimate = 2^((14 + 10) / 2) = 2^12 = 4096

  真实答案：√9437184 = 3072（即 3.0 × 1024）
  初始估值 4096 vs 真实答案 3072，偏差约 1.33 倍 → 非常接近
```

这个估值虽然粗糙，但始终把偏差控制在 2 倍以内，这对牛顿法来说是一个足够好的起点。
...
{
    if (exponent == 0) return FixedPoint.One;
...
**第二步：查二维表。** 将 (absX, absY) 归一化到查找表索引范围 [0, dim-1]，然后直接查表：
...
归一化时以较大的分量为基准，保证索引不越界，同时最大限度利用表的精度。
...
- **方法调用**：`DegreesToRadians(degrees)` 内部计算 `degrees * Pi / 180`，中间结果保留了更多位，精度更高。
