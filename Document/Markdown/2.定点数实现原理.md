...
用 `√9.0` 跟踪一轮完整的迭代过程（S = v × MULTIPLE = 9216 × 1024 = 9437184）：

```text
初始 estimate = 4096

第 1 次: (4096 + 9437184 / 4096) >> 1
       = (4096 + 2304) >> 1
       = 6400 >> 1
       = 3200

第 2 次: (3200 + 9437184 / 3200) >> 1
       = (3200 + 2949) >> 1
       = 6149 >> 1
       = 3074

第 3 次: (3074 + 9437184 / 3074) >> 1
       = (3074 + 3070) >> 1
       = 6144 >> 1
       = 3072

第 4 次: (3072 + 9437184 / 3072) >> 1
       = (3072 + 3072) >> 1
       = 6144 >> 1
       = 3072
...
第 2 次迭代就已经非常接近了，第 3 次直接收敛到精确值。实测所有 1~100 以内的完全平方数（1, 4, 9, 16, 25, 36, 49, 64, 81, 100）的 Sqrt 结果都是精确的，误差为 0。

这就是牛顿法"二次收敛"的特性——每次迭代有效精度位数翻倍。Q10 精度只需要 10 位，4 次迭代就能收敛，固定 8 次是为了留出安全余量。
...
public static FixedPoint Log2(FixedPoint value)
{
    ...
    // 第二步：迭代平方法逐位求小数部分（共 BIT_MOVE_COUNT 位）
