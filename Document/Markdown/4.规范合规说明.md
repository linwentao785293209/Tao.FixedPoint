...
库中所有算术运算符（`+` `-` `*` `/` `%` `<<` `>>`）、比较运算符、以及常用的短方法（`Abs`、`Floor`、`Saturate` 等）均标记了 `[MethodImpl(MethodImplOptions.AggressiveInlining)]`。
...
其中乘法 `operator *` 和除法 `operator /` 虽然包含溢出检测或除零检测的分支，但方法体仍然足够短（不含循环），且调用频率极高，标记内联有助于 JIT 消除调用开销。实测中分支预测器对这类"几乎不走到"的防御分支处理得很好，不会对指令流水线造成实质影响。

使用标准：

- **适用场景**：方法体很短（一般不超过 3-5 行）、调用频率极高、且不包含循环的方法。即使包含简单的 if 分支（如溢出检测），只要方法整体代码量不大，内联仍然是值得的。
- **不适用场景**：方法体较长或包含循环的方法（如 `Sqrt`、`Log2`、`Exp`）。强制内联这类方法会增大调用方的代码体积，反而可能导致指令缓存命中率下降。
- **本质是建议而非强制**：JIT 编译器本身会对短方法做内联决策，`AggressiveInlining` 只是向 JIT 表达"请优先内联此方法"的意图。对于已经足够短的方法，是否标记的性能差异通常不大，但统一标记可以保证一致性，避免某些运算符被内联而另一些没有。
